...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['department']) && !empty($_POST['department'])) {
    ...
    $sql = "
        SELECT 
            ...
            nbeds.bed_number, 
            nbeds.is_booked,
            CASE WHEN nbeds.is_booked = 1 THEN stv.Name ELSE NULL END AS student_name,
            CASE WHEN nbeds.is_booked = 1 THEN stv.Department ELSE NULL END AS student_department,
            CASE WHEN nbeds.is_booked = 1 THEN stv.Year ELSE NULL END AS student_year,
            CASE WHEN nbeds.is_booked = 1 THEN stv.Address ELSE NULL END AS student_address
        FROM 
            nblock_rooms AS nrooms
        JOIN 
            nblock_beds AS nbeds ON nrooms.room_id = nbeds.room_id
        LEFT JOIN 
            students_n AS stv ON nbeds.student_name = stv.Username
        WHERE 
            nrooms.department_id = ?
        ORDER BY 
            nrooms.room_id, nbeds.bed_number;
    ";
...
<head>
    ...
    <title>Search Rooms - N Block</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Available Rooms in N Block</h2>
                    ...
                    <ul class="beds">
                        <?php foreach ($room['beds'] as $bed): ?>
                            <li>
                                ...
                                <?php else: ?>
                                    <form action="book_bedn.php" method="POST" style="display:inline;">
        ...
        <a href="../dashboards/n.php" class="back-btn">Back to Dashboard</a>
    </div>
</body>
