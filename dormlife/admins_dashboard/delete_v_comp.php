...
if (isset($_GET['id'])) {
    ...
    $delete_query = "DELETE FROM vblock_complaints WHERE complaint_id = ?";
    $stmt = $conn->prepare($delete_query);
    $stmt->bind_param("i", $complaint_id);

    if ($stmt->execute()) {
        // Redirect back to the complaints list with a success message
        header("Location: v_comp.php?message=Complaint deleted successfully");
        exit();
    } else {
        // Redirect back with an error message
        header("Location: v_comp.php?error=Failed to delete complaint");
        exit();
    }
} else {
    // Redirect back if no ID is provided
    header("Location: v_comp.php?error=No complaint ID specified");
    exit();
}
?>