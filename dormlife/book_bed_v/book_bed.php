...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql_check = "SELECT * FROM vblock_beds WHERE bed_id = ? AND is_booked = 0";
    ...
    if ($result_check->num_rows > 0) {
        // Book the bed
        $sql_book = "UPDATE vblock_beds SET is_booked = 1, student_name = ? WHERE bed_id = ?";
        ...
        if ($stmt_book->execute()) {
            echo "<script>alert('Bed successfully booked!'); window.location.href = '../dashboards/v.php';</script>";
        } else {
