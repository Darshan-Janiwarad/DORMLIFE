...
// Check if the user is logged in and belongs to V Block
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['department']) && !empty($_POST['department'])) {
    ...
    $sql = "
        SELECT 
            ...
            vbeds.bed_number, 
            vbeds.is_booked,
            CASE WHEN vbeds.is_booked = 1 THEN stv.Name ELSE NULL END AS student_name,
            CASE WHEN vbeds.is_booked = 1 THEN stv.Department ELSE NULL END AS student_department,
            CASE WHEN vbeds.is_booked = 1 THEN stv.Year ELSE NULL END AS student_year,
            CASE WHEN vbeds.is_booked = 1 THEN stv.Address ELSE NULL END AS student_address
        FROM 
            vblock_rooms AS vrooms
        JOIN 
            vblock_beds AS vbeds ON vrooms.room_id = vbeds.room_id
        LEFT JOIN 
            students_v AS stv ON vbeds.student_name = stv.Username
        WHERE 
            vrooms.department_id = ?
        ORDER BY 
            vrooms.room_id, vbeds.bed_number;
    ";
...
<head>
    ...
    <title>Search Rooms - V Block</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Available Rooms in V Block</h2>
                    ...
                    <ul class="beds">
                        <?php foreach ($room['beds'] as $bed): ?>
                            <li>
                                ...
                                <?php else: ?>
                                    <form action="book_bed.php" method="POST" style="display:inline;">
        ...
        <a href="../dashboards/v.php" class="back-btn">Back to Dashboard</a>
    </div>
</body>
