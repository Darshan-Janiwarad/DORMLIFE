...
$user_announcements = [];
if ($stmt = $conn->prepare("SELECT * FROM n_announcements WHERE recipient = ? ORDER BY posted_at DESC")) {
...
if ($stmt = $conn->prepare("SELECT * FROM n_announcements WHERE recipient = 'all' ORDER BY posted_at DESC")) {
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
